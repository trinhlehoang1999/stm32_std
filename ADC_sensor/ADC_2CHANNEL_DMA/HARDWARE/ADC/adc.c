...
void Adc_Init(ADC_TypeDef* ADCx, uint8_t channel) {
  ...
  DMA_InitStructure.DMA_MemoryBaseAddr = (u32)&ADC_ConvertedValue;    // THAY DOI DOI GIA TRI NHAN
//	DMA_InitStructure.DMA_MemoryBaseAddr = (u32)&ADC_ConvertedValue1;
  DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralSRC;
  DMA_InitStructure.DMA_BufferSize = 2;                              // // THAY DOI DOI GIA TRI NHAN
  DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
  DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;               // // THAY DOI DOI GIA TRI NHAN(NEU CHI DUNG 1 KENH THÌ Disable)
  ...
  ADC_InitDef.ADC_NbrOfChannel = 2;	                                         // THAY DOI DOI GIA TRI NHAN
	/* Initialize ADC */
	ADC_Init(ADCx, &ADC_InitDef);
	ADC_RegularChannelConfig(ADC1, ADC_Channel_0, 1, ADC_SampleTime_1Cycles5);//   CHANNEL0
		ADC_RegularChannelConfig(ADC1,ADC_Channel_1, 2, ADC_SampleTime_1Cycles5);//  CHANNEL1
...
//	ADC_TempSensorVrefintCmd(ENABLE); // NHIET DO BEN TRONG BLUE PILL
