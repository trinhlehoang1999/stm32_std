...
uint8_t i2c_write(uint8_t u8Data)
{
	...
	delay_us(3);
	if (SDA_VAL) {
		u8Ret = 0;   // ko thaydoi chua nhan tin hieu sai dia chi
	} else {
		u8Ret = 1; // thay doi keo xuong da nhan tin hieu dung dia chi
	}
	delay_us(2);
	SCL_0;
	delay_us(5);
...
uint8_t i2c_read(uint8_t u8Ack)
{
	...
	if (u8Ack) {     //ask==0 thi se dung stop,, con ask==1 thi master tiep tuc chay.
		SDA_0;
	} else {
		SDA_1;
	}
